...
public fun MDCBanner(
  ...
  Div(attrs = {
    ...
    initialiseMDC(MDCBannerModule::MDCBanner) {
      if (open) open() else close(MDCBannerModule.CloseReason.UNSPECIFIED)
    }
    applyAttrs(attrs)
  }, content = {
    MDCSideEffect<MDCBannerModule.MDCBanner>(open, centered, mobileStacked) {
      if (open) open() else close(MDCBannerModule.CloseReason.UNSPECIFIED)
    }
    if (fixed) {
      Div(attrs = {
